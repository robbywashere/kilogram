...
describe('Tasks', ()=>{
  ...
  const password = '!#@$%', username = 'heydude';
  ...
  describe('downloadIGAva', ()=>{
    ...
    it ('should download igaccount avatar', async ()=>{
      ...
      assert.equal(reqPipe.get.getCall(0).args[0], 'https://instagram.fdad1-1.fna.fbcdn.net/vp/fb2f9813830c704783a165569b95a6ff/5BEB2087/t51.2885-19/s150x150/35414722_1833801803589327_7624906533519753216_n.jpg');
  ...
  describe('verifyIG', ()=>{
    ...
    it ('should emit IGAccount:good & job:complete on successful deviceAgent login', async ()=>{
      ...
      Tasks.verifyIG({ events, jobId: 1, jobName: 'verifyIG', agent, IGAccount });
    ...
    it('should emit IGAccount:fail & job:complete on known failed state of deviceAgent login', async ()=>{
      ...
      const agent = { exec: sinon.spy(async ()=> ({ body: { login: false, type: 'creds_error' } }))};
  ...
  describe('sendEmail',()=>{
    it ('should emit job:complete upon email send', async ()=>{
      ...
      const data = { to: 'a@a.com', from: 'b@b.com', msg: 'hi', subject: 'hi' };

      Tasks.sendEmail({ events, jobId: 1, jobName: 'email', data });
  ...
  describe('post', ()=>{
    ...
    it ('should emit Post:published and job:complete on successful deviceAgent login and post', async ()=>{
      ...
      Tasks.post({ events, jobId: 1, jobName: 'post', agent, IGAccount, Post, minioClient });
    ...
    it ('should emit job:error on unknown deviceAgent login error and post attempt', async ()=>{
      ...
      const agent = {
        exec: async () => ({
          body: {
            success: false, login: false, type: '?????',
          },
        }),
      };
