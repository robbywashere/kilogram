...
async function pgSchemaDump({ username, database, name = 'public' }) {
  ...
  for (const row of await dumpFunctionsReadOnly({ namespace: name })) {
    output += '\n';
...
async function pgSchemaDumpFile({
  username, database, path, name = 'public',
}) {
  const output = await pgSchemaDump({ username, database, name });
  writeFileSync(path, output);
}

async function pgSchemaDumpCompare({
  username, database, path, name = 'public',
}) {
