...
describe('objectLoader', function(){
  ...
  beforeEach(()=>DBSync(true));
    ...
    const testObj = {
      ...
      Methods:{
        instanceMeth: function (){ return true },
      },
      StaticMethods: {
        staticMeth: function (){ return true },
      },
      Init(objs){
        InitObjs = objs;
      },
  ...
  it ('should call Init() function with registry', function(){
    assert(InitObjs.TestObj.prototype instanceof Model)
  })

  it ('should make instance and static methods', async function(){
    assert(TestObj.staticMeth())
    const to = await TestObj.create({});
    assert(to.instanceMeth())
  })

  it ('should turn scopes into functions with ScopeFunctions: true', async function(){
