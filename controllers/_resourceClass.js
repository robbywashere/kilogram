...
class BaseResource {
  ...
  callScope(user){
    return this.policy.scope({ user, model: this.model })
  }
  ...
  action(name,opts={}){
    return async (req, res, next) => {
      try {
        ...
        //
        //policyFn.__proto__.constructor.name === "AsyncFunction" do await
        if (await policyFn()) {
          if (instance.save) {
  ...
  async show({ user, params }){
    const resource = await this.callScope(user).findById(params.id);
    if (!resource) throw new NotFound();
    return resource;
  }

  async index({ user },opts){
    return this.callScope(user).findAndCountAll(opts);
  }
