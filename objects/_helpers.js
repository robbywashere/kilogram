...
function denormalizeJobBody(objectRegistry, body) {
  ...
  for (const [modelName, id] of Object.entries(body)) {
    if (!objectRegistry[modelName]) {
      throw new Error(`Model ${modelName} relation does not exist, cannot denormalize body ->\n ${JSON.stringify(
        body,
        null,
        4,
      )}`);
  ...
  return Promise.all(queries.map(query => query())).then(resultingArray =>
    zipObject(Object.keys(body), resultingArray.map(r => r.toJSON())));
}
